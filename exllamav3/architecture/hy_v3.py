...
from .hy_v3_mtp import HyV3MTPModel

class HyV3Config(Config):
    arch_string = "HYV3ForCausalLM"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            directory,
            {"text": HyV3Model, "mtp": HyV3MTPModel},
            **kwargs
        )
        ...
        self.use_qk_norm = self.read_cfg(bool, "qk_norm", True)
        ...
        self.assert_cfg(bool, "moe_router_use_sigmoid", True, True)
        self.assert_cfg(bool, "moe_router_enable_expert_bias", True, True)
        self.assert_cfg(bool, "route_norm", True, True)
        self.intermediate_size = self.read_cfg(int, "intermediate_size", no_default)
        self.moe_intermediate_size = self.read_cfg(int, "moe_intermediate_size", no_default)
        self.num_shared_experts = self.read_cfg(int, "num_shared_experts", 1)
        self.num_experts = self.read_cfg(int, "num_experts", 192)
        self.num_experts_per_tok = self.read_cfg(int, "num_experts_per_tok", 8)
        self.first_k_dense_replace = self.read_cfg(int, "first_k_dense_replace", 1)
        self.routed_scaling_factor = self.read_cfg(float, "router_scaling_factor", 2.826)
...
class HyV3Model(Model):
                ...
                mlp = (
                    ...
                    BlockSparseMLP(
                        ...
                        shared_experts = GatedMLP(
                            config = config,
                            key = f"{key_prefix}.layers.{idx}.mlp.shared_mlp",
                            hidden_size = config.hidden_size,
    ...
    def default_chat_prompt(self, prompt: str, system_prompt: str = None) -> str:
        p = f"<｜hy_begin_of_sentence:opensource｜>"
        if system_prompt:
            p += system_prompt
        p += f"<｜hy_User:opensource｜>{prompt}"
        p += f"<｜hy_Assistant:opensource｜>"
        return p
