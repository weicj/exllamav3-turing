            ...
            for idx, module in enumerate(modules):
                ...
                while True:
                    try:
                        ...
                        fail = False
                        progress.update(idx + 1)
                        if os.environ.get("EXL3_AUTOSPLIT_TRACE") == "1" and load_device.type == "cuda":
                            free, total = torch.cuda.mem_get_info(load_device)
                            allocated = torch.cuda.memory_allocated(load_device)
                            reserved = torch.cuda.memory_reserved(load_device)
                            print(
                                f"AUTOSPLIT_LOADED index={idx} module={module.key} device={load_device.index} "
                                f"allocated={allocated} reserved={reserved} free={free} total={total}",
                                flush=True,
                            )
                    ...
                    # We're not good
                        ...
                        if e.__class__.__name__ == "OutOfMemoryError" or \
                            ...
                            if os.environ.get("EXL3_AUTOSPLIT_TRACE") == "1":
                                print(
                                    f"AUTOSPLIT_OOM index={idx} module={module.key} "
                                    f"device={load_device.index if load_device.type == 'cuda' else 'cpu'} "
                                    f"error={e}",
                                    flush=True,
                                )
