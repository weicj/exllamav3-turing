...
_trace_stop_after_module = int(os.environ.get("EXL3_TRACE_STOP_AFTER_MODULE", "-1"))
...
# Worker acks are used by the parent as a lifetime fence for shared input/KV
...
def _save_boundary_trace(local_context: dict, phase: str, pass_idx: int, stage: str, tensor: torch.Tensor):
    """Hash the TP handoff and first module boundaries without retaining full activations."""
    if _boundary_trace_path is None:
        return
    if phase != _boundary_trace_phase or (_boundary_trace_pass >= 0 and pass_idx != _boundary_trace_pass):
        return
    ...
    with open(f"{_boundary_trace_path}.r{local_context['rank']}", "a") as handle:
        handle.write(json.dumps(row, sort_keys = True) + "\n")


def _save_module_trace(kind: str, device: int, pass_idx: int, module_idx: int, module, x: torch.Tensor):
    """Persist one module boundary for PP/TP numerical localization.

    This is deliberately opt-in and lives only in the TP experiment runtime. A CPU copy keeps
    the trace independent of allocator reuse in the worker process.
    """
    if _trace_dir is None or x is None:
        return
    os.makedirs(_trace_dir, exist_ok = True)
    path = os.path.join(
        _trace_dir,
        f"{kind}-d{device}-p{pass_idx:02d}-m{module_idx:03d}.pt"
    )
    torch.save({
        "key": module.key,
        "shape": tuple(x.shape),
        "dtype": str(x.dtype),
        "tensor": x.detach().float().cpu().contiguous(),
    }, path)
...
def _save_module_profile(local_context: dict, phase: str, pass_idx: int, events: list):
    ...
    path = os.path.join(
        _module_profile_dir,
        f"module-profile-rank{local_context['rank']}-{phase}-p{pass_idx:03d}.json",
    )
...
def mp_model_worker(
    ...
    with torch.inference_mode():
        ...
        # NCCL initializes CUDA before this worker opens the shared arena. On this driver a
        # post-NCCL cudaHostRegister is invalid, so retain pinned input staging only for native.
        pin_memory = backend_args["type"] != "nccl"
        local_context["inf_consumer"] = SMConsumer(producer, device = device, pin_memory = pin_memory)
        ...
        while True:
            ...
            try:
                ...
                _sync_after_cuda_call(func, device)
                conn.send(result)
...
def mp_model_forward(
    ...
    pass_idx = local_context.get("module_trace_pass", 0)
    local_context["module_trace_pass"] = pass_idx + 1
    trace_this_pass = _trace_dir is not None and _trace_phase == ("prefill" if prefill else "decode") \
        and pass_idx == _trace_pass
    ...
    for idx, module in enumerate(modules):
        ...
        if _module_profile_dir is not None:
            backend.set_profile_module(module.key)
        if profile_this_pass:
        ...
        if _module_profile_dir is not None:
            backend.set_profile_module(None)
        if profile_this_pass:
            end.record()
            module_events.append((idx, module.key, start, end))
        if trace_this_pass:
            _save_module_trace("tp", local_context["device"], pass_idx, idx, module, x)
...
class PseudoParentConn:
    ...
    def __init__(
        ...
        pin_memory = backend_args["type"] != "nccl"
        self.local_context["inf_consumer"] = SMConsumer(producer, device = device, pin_memory = pin_memory)
        self.result = None
        self.device = device
    ...
    def send(self, msg):
        if msg == "quit":
            ...
            _sync_after_cuda_call(fn, self.device)
