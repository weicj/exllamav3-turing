...
class SafetensorsCollection:
    ...
    def _get_tensor_file_map_trie(self):
        try:
            ...
            # TP planning needs prefix lookup before any model tensor is loaded. Keep the
            # optional C-extension an optimization rather than a runtime requirement; the
            # checkpoint key set is small enough that a cached Python tuple is sufficient for
            # this one-time allocation pass.
            class PrefixIndex:
                ...
                def keys(self, prefix):
                    ...
                    first = bisect_left(self._keys, prefix)
                    last = bisect_left(self._keys, prefix + chr(0x10ffff))
                    return list(self._keys[first:last])

            return PrefixIndex(self.tensor_file_map.keys())
        return marisa_trie.Trie(self.tensor_file_map.keys())
