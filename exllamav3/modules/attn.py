...
_attn_subtrace_path = os.environ.get("EXL3_ATTN_SUBTRACE_JSONL")
_attn_subtrace_layers = {
    int(value) for value in os.environ.get("EXL3_ATTN_SUBTRACE_LAYERS", "").split(",") if value
}
_attn_subtrace_calls = {}
...
def _save_attn_subtrace(module, stage: str, tensor: torch.Tensor | None):
    """Record exact attention boundaries for TP determinism localization."""
    if _attn_subtrace_path is None or tensor is None or (
        _attn_subtrace_layers and module.layer_idx not in _attn_subtrace_layers
    ):
        return
    if stage == "input":
        call = _attn_subtrace_calls.get(module.key, 0)
        _attn_subtrace_calls[module.key] = call + 1
        _attn_subtrace_current_calls[module.key] = call
    else:
        call = _attn_subtrace_current_calls.get(module.key, -1)
    ...
    with open(f"{_attn_subtrace_path}.d{tensor.device.index}", "a") as handle:
        handle.write(json.dumps(row, sort_keys = True) + "\n")


def _cache_trace_hash(tensor: torch.Tensor | None, pages: torch.Tensor | None = None) -> str:
    if tensor is None:
        return "none"
...
def _trace_cache_pages(attn, cache, block_table, cache_seqlens, qlen: int, tag: str):
    if not _cache_trace or attn.layer_idx != _cache_trace_layer:
        return
    if cache is None or cache_seqlens is None or block_table is None:
        return
    try:
        torch.cuda.synchronize(attn.device)
        n_pages = (int(cache_seqlens[0].item()) + qlen + PAGE_SIZE - 1) // PAGE_SIZE
        pages = block_table[0, :n_pages].long()
        k_cache, v_cache = cache.get_kv(cache_seqlens, block_table)
        print(
            f"CACHE_TRACE layer={attn.layer_idx} tag={tag} qlen={qlen} "
            f"seqlen={int(cache_seqlens[0].item())} pages={pages.detach().cpu().tolist()} "
            f"kh={_cache_trace_hash(k_cache, pages)} vh={_cache_trace_hash(v_cache, pages)}",
            flush = True,
        )
    except Exception as e:
        print(f"CACHE_TRACE_ERROR layer={attn.layer_idx} tag={tag} error={e!r}", flush = True)
...
class Attention(Module):
    ...
    def __init__(
        ...
        # `attn_dispatch` otherwise has no module identity. Keep the layer index beside
        # its fast-path hint so a diagnostic backend can be scoped to one layer.
        self.dispatch_cache = {"layer_idx": self.layer_idx}
        self.bc_attn = {}
    ...
    def forward(
        ...
        _save_attn_subtrace(self, "input", x)
        if self.num_kv_heads == 0:
            ...
            match attn_mode:
                ...
                case _:
                    raise ValueError(f"Unknown attn_mode: {attn_mode}")
            _save_attn_subtrace(self, "o_pre_reduce", x)
            if self.tp_reduce:
                params["backend"].all_reduce(x)
        _save_attn_subtrace(self, "output", x)
    ...
    def decode_flash_attn_nc(
        ...
        _save_attn_subtrace(self, "q", q)
        _save_attn_subtrace(self, "k", k)
        _save_attn_subtrace(self, "v", v)

        if _cache_trace and int(cache_seqlens[0].item()) >= 3000:
            _trace_cache_pages(self, cache, block_table, cache_seqlens, seqlen, "before")
        ...
        _save_attn_subtrace(self, "attn_o", o)

        if _cache_trace and int(cache_seqlens[0].item()) >= 3000:
            _trace_cache_pages(self, cache, block_table, cache_seqlens, seqlen, "after")
    ...
    def decode_flash_attn(
        ...
        if (
            ...
            not (qsa_sparse_disabled and self.qsa_indexer is not None) and
