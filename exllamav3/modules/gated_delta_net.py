...
_disable_bc_gdn = os.environ.get("EXL3_DISABLE_BC_GDN", "0") == "1"
_skip_recurrent_clear_sync = os.environ.get("EXL3_SKIP_RECURRENT_CLEAR_SYNC", "0") == "1"
...
def _save_gdn_subtrace(module, stage: str, tensor: torch.Tensor):
    ...
    def __init__(
        ...
        if not exported:
            ...
            if clear and stashed is None:
                ...
                if not _skip_recurrent_clear_sync:
                    # FLA's prefill extension may consume this state on an internal stream.
                    # Its first read must not race the state clear submitted above. This fence
                    # runs only once when a new recurrent slot is acquired, never per token.
                    devices = {torch.device(l.device).index for l in self.cache.get_all_recurrent_layers().values()
                               if l.device is not None and torch.device(l.device).type == "cuda"}
...
class GatedDeltaNet(Module):
    ...
    def forward(
        ...
        if (
            not _disable_bc_gdn and self.bc_split and save_state and
        ...
        if self.qkvz_proj is not None and self.ba_proj is not None:
            ...
            _save_gdn_subtrace(self, "qkv", qkv)
            _save_gdn_subtrace(self, "z", z)
            _save_gdn_subtrace(self, "b", b)
            _save_gdn_subtrace(self, "a", a)
        ...
        _save_gdn_subtrace(self, "norm", core_attn_out)

        # Output projection
        x = self.o_proj.forward(core_attn_out, params)
        _save_gdn_subtrace(self, "o_pre_reduce", x)

        # TP reduction
        if self.tp_reduce:
            params["backend"].all_reduce(x)
        _save_gdn_subtrace(self, "output", x)
