...
_ple_trace_path = os.environ.get("EXL3_PLE_TRACE_JSONL")
_ple_trace_calls = {}
_ple_trace_current_calls = {}


def _save_ple_trace(module, stage: str, tensor: torch.Tensor | None):
    """Hash PLE boundaries for a same-load determinism investigation."""
    if _ple_trace_path is None or tensor is None:
        return
    if stage == "input":
        call = _ple_trace_calls.get(module.key, 0)
        _ple_trace_calls[module.key] = call + 1
        _ple_trace_current_calls[module.key] = call
    else:
        call = _ple_trace_current_calls.get(module.key, -1)
    ...
    with open(f"{_ple_trace_path}.d{tensor.device.index}", "a") as handle:
...
class PLELayer(Module):
    ...
    def make_tp_allocation(self, options: dict):
        # The PLE projection math and stream residual are identical on every TP rank. Keep the
        # small projections replicated rather than introducing extra all-reduces; the 24 GiB
        # n-gram table itself remains disk streamed by NGramEmbedding.tp_import().
        return []

    def load_local(self, device):
        for recurrent_layer in self.recurrent_layers:
            recurrent_layer.alloc(device)

    def tp_export(self, plan, producer):
        if self.ple_embedding.mode not in ("trellis_disk", "fp16_disk"):
            raise RuntimeError("Tensor-parallel PLE requires EXL3_NGRAM_STREAM=1")
        return {
            "cls": PLELayer,
            "kwargs": {
                "key": self.key,
                ...
                "ple_embed_dim": self.ple_embedding.ple_embed_dim,
            ...
            "ple_embedding": self.ple_embedding.tp_export(plan, producer),
            "key_proj": self.key_proj.tp_export(plan, producer),
            "value_proj": self.value_proj.tp_export(plan, producer),
            "norm_key": self.norm_key.tp_export(plan, producer),
            "norm_query": self.norm_query.tp_export(plan, producer),
            "norm_conv": self.norm_conv.tp_export(plan, producer),
            "conv_w": producer.send(self.conv_w),
            "recurrent_layers": [
                recurrent_layer.tp_export(plan) for recurrent_layer in self.recurrent_layers
            ],
            "device": self.device,
        }
    ...
    def tp_import(local_context, exported, plan):
        ...
        def import_replicated_linear(name):
            value = exported[name]
            return value["cls"].tp_import_split(local_context, value, plan, None)

        def import_module(name):
            value = exported[name]
            return value["cls"].tp_import(local_context, value, plan)

        module = PLELayer(config = None, **exported["kwargs"])
        module.ple_embedding = import_module("ple_embedding")
        module.key_proj = import_replicated_linear("key_proj")
        module.value_proj = import_replicated_linear("value_proj")
        module.norm_key = import_module("norm_key")
        module.norm_query = import_module("norm_query")
        module.norm_conv = import_module("norm_conv")
        module.modules = []
        for child in (
            module.ple_embedding,
            module.key_proj,
            module.value_proj,
            module.norm_key,
            module.norm_query,
        ...
        for recurrent_layer in exported["recurrent_layers"]:
            imported = recurrent_layer["cls"](module, **recurrent_layer["args"])
            module.recurrent_layers.append(imported)
            module.tp_recurrent_lookup[recurrent_layer["args"]["cache_id"]] = imported
    ...
    def forward_streams(
        ...
        _save_ple_trace(self, "embedding", emb)
        if self.key_proj.quant_type == "fp16" and self.value_proj.quant_type == "fp16" \
            ...
            _save_ple_trace(self, "delta", delta)
            _save_ple_trace(self, "conv_stream", conv_stream)
            return delta, conv_stream
        delta, conv_stream = self.forward_streams_reference(streams, emb, params, conv_state)
        _save_ple_trace(self, "delta", delta)
        _save_ple_trace(self, "conv_stream", conv_stream)
        return delta, conv_stream
    ...
    def forward(self, x: torch.Tensor, params: dict, out_dtype: torch.dtype | None = None):
        ...
        if rsg:
            ...
            # TP workers receive exported GDNState handles whose cache field is the cache id;
            # their local PLE state lives in the module's id -> state lookup, just as GDN does.
            if isinstance(rsg[0].cache, int):
            ...
            _save_ple_trace(self, "history", history)
            _save_ple_trace(self, "conv_state", cs)
            ...
            history = torch.cat((pad, ids), dim = 1)
            _save_ple_trace(self, "history", history)
            delta, _ = self.forward_streams(x, history, params)
        out = x + delta
        _save_ple_trace(self, "output", out)
        return out
