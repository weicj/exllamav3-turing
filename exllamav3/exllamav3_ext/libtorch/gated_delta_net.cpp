...
// Keep this limited to the split GDN fast path so quality experiments can
// isolate graph replay without disabling the rest of the decode acceleration.
static bool disable_bc_gdn_graph()
{
    const char* value = std::getenv("EXL3_DISABLE_BC_GDN_GRAPH");
...
)
{
    ...
    if (disable_bc_gdn_graph() || s.graph->disabled ||
        (!s.graph->ready && !s.graph->ready_to_record))
    {
