"""Run identical greedy PP2 jobs in one model load to expose state-reset drift."""
...
EXPECTED_GPUS = int(os.environ.get("BENCH_EXPECTED_GPUS", "2"))
PARALLELISM = os.environ.get("BENCH_PARALLELISM", "PP2 layer split")
...
def run_job(generator, prompt_tokens, output_tokens, seed):
    ...
    return {
        ...
        "state_slots_free_after_job": list(generator.cache.free_list),
    }


def clear_request_cache(generator):
    """Force the next probe request to run from an empty KV/recurrent state."""
...
def main():
    ...
    if torch.cuda.device_count() != EXPECTED_GPUS:
        raise RuntimeError(f"expected {EXPECTED_GPUS} visible GPUs, got {torch.cuda.device_count()}")
    ...
    try:
        ...
        print("PP2_SAME_LOAD_JSON=" + json.dumps(report, sort_keys=True), flush=True)
    finally:
        model.unload()
